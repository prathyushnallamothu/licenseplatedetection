...
def convert_to_yolov5(info_dict):
    ...
    save_file_name = os.path.join(
        "data/labels", info_dict["filename"].replace("png", "txt"))
...
# Get the annotations
annotations = [os.path.join('data/annotations', x)
               for x in os.listdir('data/annotations') if x[-3:] == "xml"]
annotations.sort()
...
annotations = [os.path.join('data/annotations', x)
               for x in os.listdir('data/annotations') if x[-3:] == "txt"]
